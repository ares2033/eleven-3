...
export function Globe({ globeConfig, data }: Readonly<WorldProps>) {
  ...
  useEffect(() => {
    ...
    // Helper to check if two points have the same lat and lng
    ...
    const filteredPoints = points.filter(
      (v, i, a) => a.findIndex((v2) => isSameLatLng(v2, v)) === i,
    );
    ...
    globeRef.current
      ...
      .arcStroke((): number | null => {
        const values: number[] = [0.32, 0.28, 0.3];
        const idx: number = Math.round(Math.random() * 2);
